alter table public.rentcam_orders add column if not exists discount jsonb not null default '{}';
...
DO $test$ declare v_id uuid;v_token uuid;p jsonb;o rentcam_orders;doc jsonb; begin
begin
...
p=jsonb_build_object('id',v_id,'token',v_token,'name','TEST DISCOUNT ROLLBACK','phone','081234567899','start',current_date+10,'end',current_date+12,'discount_mode','product','items',jsonb_build_array(jsonb_build_object('id','arri-alexa-35','q',2,'discount_percent',25)));
doc=rentcam_private.pro_action('quotation',p);
select * into o from rentcam_orders where id=v_id;
if o.total<>round((o.items->0->>'original_amount')::numeric*.75) then raise exception 'Product discount wrong';end if;
perform rentcam_private.pro_action('quotation',p);
if (select total from rentcam_orders where id=v_id)<>o.total then raise exception 'Retry compounded discount';end if;
v_id=gen_random_uuid();p=p||jsonb_build_object('id',v_id,'token',gen_random_uuid(),'discount_mode','days','free_days',1);
perform rentcam_private.pro_action('quotation',p);
select * into o from rentcam_orders where id=v_id;
if (o.items->0->>'charged_days')::int<>2 or o.total<>round((o.items->0->>'original_amount')::numeric*2/3) then raise exception 'Days discount wrong';end if;
raise exception using errcode='P0002',message='rollback successful tests';

...
(function(){
  ...
  const templates={
    ...
    allRental:{
      ...
      mainCategories:allRentalCategories,
  ...
  function syncWebsiteContent(){
    ...
    allRentalCategories.forEach(([id,name,sort])=>{if(!c.mainCategories.some(x=>String(x.id)===id))c.mainCategories.push({id,name,sort,active:true})});
    ...
    allRentalCategories.forEach(([id,name])=>{if(!c.subCategories.some(x=>String(x.mainCategory)===id))c.subCategories.push({id:id+'-utama',name:name+' Utama',mainCategory:id,sort:10,active:true})});
  ...
  function ensureAllRentalCategories(){
    ...
    c.mainCategories=[...(c.mainCategories||[])];
    c.subCategories=[...(c.subCategories||[])];
    allRentalCategories.forEach(([id,name,sort])=>{

...
(()=>{
  ...
  function wa(order,customer,items,x,link){
    ...
    const lines=[`Halo ${window.RENTCAM_CMS_CONFIG?.general?.siteName||'Rentcam'}, saya mau order rental.`,'','*ORDER WEB*',`No: ${order.order_number||'-'}`,`Customer: ${customer.name||'Belum diisi'}`,`WhatsApp: ${customer.phone?('+'+customer.phone):'Belum diisi'}`,`Email: ${customer.email||'Belum diisi'}`,'','*DAFTAR PRODUK*',...items.map((i,n)=>`${n+1}. ${pname(i.id)}\n   Qty: ${Number(i.q)||1}`),'','*JADWAL*',`Mulai: ${x.start||'Belum diisi'}`,`Selesai: ${x.end||'Belum diisi'}`,`Terima alat: ${x.mode==='delivery'?'Diantar ke lokasi':'Ambil sendiri'}`,`Jam terima: ${wib(x.deliver_at)}`,`Kembali: ${x.return_mode==='collect'?'Dijemput dari lokasi':'Kembalikan sendiri'}`,`Jam kembali: ${wib(x.collect_at)}`,'','*LOKASI*',`Alamat: ${x.address||'Belum diisi'}`,`Google Maps: ${x.maps_url||'Belum diisi'}`];
    if(x.notes)lines.push('','*CATATAN*',x.notes);
    lines.push('','*TOTAL ESTIMASI*',M(order.total),'',`Link status: ${location.origin}${link}`);
  ...
  function mount(){if(location.pathname!=='/cart')return;const host=document.querySelector('#app .summary');if(!host||host.querySelector('#rentcamOrderForm'))return;host.querySelector('.wide')?.remove();host.insertAdjacentHTML('beforeend',formHtml());update(host.querySelector('form'))}

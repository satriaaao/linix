(function(root,factory){
  ...
  function parseLightingComplaint(text,opts){
    ...
    var max=Math.max(5,Number(opts.maxLightingComplaintRows||9)),slides=[];for(var i=0;i<data.length;i+=max)slides.push({template:'lighting-complaint',reportType:'gudang-lighting',department:'LIGHTING',groups:[],lightingComplaints:data.slice(i,i+max),pageNo:Math.floor(i/max)+1,pageTotal:Math.max(1,Math.ceil(data.length/max)),period:opts.period||'JUNI 2026'});return {kind:'lighting-complaint',slides:slides};
  }

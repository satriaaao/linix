(function(root,factory){
  ...
  function renderSlide(report){
    ...
    var top=data.slice().sort(function(a,b){return b.values.reduce(function(s,v){return s+(v||0);},0)-a.values.reduce(function(s,v){return s+(v||0);},0);}).slice(0,5);
    var max=Math.max(1,...top.flatMap(function(r){return r.values.map(function(v){return v||0;});}));
    var colors=['#087d86','#2563eb','#a21caf','#15803d','#b91c1c'];
    var chart='<svg viewBox="0 0 1460 180" role="img" aria-label="Grafik pendapatan bulanan produk">';
    top.forEach(function(r,k){var d='',open=false;r.values.forEach(function(v,j){if(v==null){open=false;return;}var x=40+j*125,y=145-v/max*120;d+=(open?' L':'M')+x+' '+y;open=true;chart+='<circle tabindex="0" role="button" aria-label="'+esc(r.name+' '+MONTHS[j]+': Rp'+v.toLocaleString('id-ID'))+'" data-detail="'+esc(r.name+' | '+MONTHS[j]+' | Rp'+v.toLocaleString('id-ID'))+'" cx="'+x+'" cy="'+y+'" r="5" fill="'+colors[k]+'"><title>'+esc(r.name+' '+MONTHS[j]+': Rp'+v.toLocaleString('id-ID'))+'</title></circle>';});chart+='<path d="'+d+'" fill="none" stroke="'+colors[k]+'" stroke-width="2" pointer-events="none"/>';});
    MONTHS.forEach(function(m,j){chart+='<text x="'+(40+j*125)+'" y="175" text-anchor="middle">'+m+'</text>';});chart+='</svg>';
    return '<section class="it-product-slide it-products-full"><div class="omset-kicker">LAPORAN IT <span>2026 / '+(report.pageNo||1)+' dari '+(report.pageTotal||1)+'</span></div><h1>Analisis Produk <span>2026</span></h1><p>Januari–Desember · Rupiah · Persentase dibanding bulan sebelumnya</p><div class="it-products-chart">'+chart+'</div><div class="it-products-detail" aria-live="polite">Tren 5 produk terbesar pada halaman ini</div><table class="it-products-table"><thead><tr><th>No</th><th>Nama Produk</th>'+MONTHS.map(function(m){return '<th>'+m+'</th>';}).join('')+'<th>Status '+(latest>=0?MONTHS[latest]:'')+'</th></tr></thead><tbody>'+rows+'</tbody></table></section>';
  }

(function(root,factory){
  ...
  function createSlides(products){
    ...
    for(var i=0;i<products.length;i+=size)slides.push({template:'it-product',reportType:'it',department:'IT',subtitle:'Analisis Produk 2026 ('+(slides.length+1)+'/'+total+')',month:12,year:2026,groups:[],products:products.slice(i,i+size),pageNo:slides.length+1,pageTotal:total,latestMonth:latestMonth});
    return slides;
  }
  ...
  function renderSlide(report){
    ...
    var visibleMonths=latest>=0?MONTHS.slice(0,latest+1):[];
    var rows=data.map(function(r,i){
      var status=growth(r.values[latest-1],r.values[latest]);
      return '<tr><td>'+esc(r.no||((report.pageNo-1)*PAGE_SIZE+i+1))+'</td><th scope="row">'+esc(r.name)+'</th>'+visibleMonths.map(function(m,j){var v=r.values[j],g=growth(r.values[j-1],v);return '<td title="'+esc(v==null?'':'Rp'+v.toLocaleString('id-ID'))+'">'+(v==null?'':v.toLocaleString('id-ID',{maximumFractionDigits:0})+'<small class="'+g.tone+'">'+esc(g.label)+'</small>')+'</td>';}).join('')+'<td class="'+status.tone+'">'+status.status+'</td></tr>';
    }).join('');
    var period=latest>=0?'Januari–'+MONTHS[latest]:'Januari';
    return '<section class="it-product-slide it-products-full"><div class="omset-kicker">LAPORAN IT <span>2026 / '+(report.pageNo||1)+' dari '+(report.pageTotal||1)+'</span></div><h1>Analisis Produk <span>2026</span></h1><p>'+period+' · Rupiah · Persentase dibanding bulan sebelumnya</p><table class="it-products-table"><thead><tr><th>No</th><th>Nama Produk</th>'+visibleMonths.map(function(m){return '<th>'+m+'</th>';}).join('')+'<th>Status '+(latest>=0?MONTHS[latest]:'')+'</th></tr></thead><tbody>'+rows+'</tbody></table></section>';
  }

/* Rich privacy-preserving analytics dashboard for Rentcam CMS. */
(function(){
  ...
  async function ensureLeaflet(){
    ...
    leafletPromise=new Promise((resolve,reject)=>{
      ...
      const old=document.querySelector('script[data-rc-leaflet]');if(old){old.addEventListener('load',()=>resolve(window.L),{once:true});old.addEventListener('error',reject,{once:true});return}
  ...
  async function upgrade(){
    const host=document.getElementById('v5report');if(!host||host.dataset.geoLoading==='1'||host.querySelector('[data-geo-analytics]'))return;
  ...
  setInterval(()=>{if(document.getElementById('v5report')){const h=document.getElementById('v5report');h.querySelector('[data-geo-analytics]')?.remove();schedule()}},30000);
  schedule();
})();

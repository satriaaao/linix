...
(function(root,factory){
  ...
  function addAdminHeaders(inputHeaders,session,password){
    const h=new Headers(inputHeaders||{});
    if(session?.token)h.set('x-rentcam-session',session.token);
    if(password)h.set('x-rentcam-admin',password);
    return h;
  }
  function create(env,opts={}){
    ...
    function password(){return sessionStore.getItem(cfg.passKey)||''}
    ...
    async function login(email,pass){
      ...
      sessionStore.setItem(cfg.passKey,pass);
      const s=await issueSession(pass);
      local.setItem(cfg.authKey,JSON.stringify({access_token:'rentcam-cms-custom',token_type:'bearer',expires_in:2592000,user:{email:cfg.adminEmail}}));
      return s;
    }
    async function ensureSession(){
      ...
      if(s)return s;
      const pass=password();
      if(pass){try{return await issueSession(pass)}catch(_){clearSession()}}
    ...
    async function request(input,init={}){
      ...
      const s=readSession();
      const pass=password();
      const headers=addAdminHeaders(init.headers||input?.headers,s,pass);
      ...
      env.fetch=async function(input,init={}){
        ...
        if(isLoginRequest(url,method,cfg.sb)){
          ...
          try{
            ...
            return new Response(JSON.stringify({access_token:'rentcam-cms-custom',token_type:'bearer',expires_in:2592000,user:{email:cfg.adminEmail}}),{status:200,headers:{'Content-Type':'application/json'}});
          }catch(e){
            clearSession();

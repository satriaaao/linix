function manifest(req,res){
  ...
  const body={
    ...
    icons:[
      {src:'/api/cms?asset=icon',sizes:'any',type:'image/svg+xml',purpose:'any'},
      {src:'/api/cms?asset=icon',sizes:'any',type:'image/svg+xml',purpose:'maskable'}
    ]
  };
  res.statusCode=200;
...
self.addEventListener('message',function(event){
  ...
  event.waitUntil(self.registration.showNotification(d.title||'Order Baru', {
    ...
    icon:'/api/cms?asset=icon',
    badge:'/api/cms?asset=icon',
...
async function registerSW(){
  ...
  try{
    swReg=await navigator.serviceWorker.register('/api/cms?asset=sw',{scope:'/cms'});
...
async function showSystemNotification(order,test){
  ...
  else await reg.showNotification(title,{body:body,icon:'/api/cms?asset=icon',badge:'/api/cms?asset=icon',tag:payload.tag,renotify:true,silent:false,data:{url:'/cms'}});
}

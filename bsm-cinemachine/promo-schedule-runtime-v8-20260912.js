...
#app .rc-badges{position:absolute;top:12px;right:auto;left:12px;display:flex;align-items:flex-start;flex-direction:column;gap:6px;z-index:25;pointer-events:none}
...
@media(max-width:620px){#app .rc-badges{top:8px;right:auto;left:8px;gap:4px}#app .rc-label{font-size:8px;padding:6px 7px}#app .rc-price{font-size:13px;gap:5px}}
...
function patchDetail(){const id=location.pathname.match(/^\/produk\/([^/?#]+)/)?.[1];if(!id)return;const s=state(decodeURIComponent(id)),info=document.querySelector('#app .detail-info');if(!info)return;const holder=document.querySelector('#app .cms-gallery-main,#app .detail-media');info.querySelectorAll('.rc-detail-badges').forEach(x=>x.remove());if(holder){holder.style.position='relative';let box=holder.querySelector('.rc-badges');if(!box){box=document.createElement('div');box.className='rc-badges';holder.appendChild(box)}setMarkup(box,badges(s))}const price=info.querySelector('.detail-price');if(price){price.classList.add('rc-price');setMarkup(price,prices(s))}info.querySelectorAll('button[onclick*="add("]').forEach(b=>{b.disabled=s.stopped;b.classList.toggle('rc-disabled',s.stopped)})}
...
function run(){style();document.querySelectorAll('#app .arri-source-badge,#app .home-category-badge,#app .related-card-badge').forEach(el=>el.remove());document.querySelectorAll('#app .pcard,#app .home-category-card').forEach(patchCard);patchDetail();patchHero()}
